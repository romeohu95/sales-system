...
$sub = $_GET['subcategoria_id'] ?? 0;
$q = $_GET['q'] ?? '';
$sql = $cn->prepare("SELECT id, nombre FROM subcategorias_especificas WHERE subcategoria_id = ? AND nombre LIKE CONCAT('%',?,'%') ORDER BY nombre LIMIT 50");
$sql->bind_param("is",$sub,$q);
$sql->execute();
$res = $sql->get_result();
$out=[];
